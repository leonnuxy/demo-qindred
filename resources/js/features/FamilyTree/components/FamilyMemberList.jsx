    ...
    <div className="family-list grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {membersList.map((member) => (
        <div
          ...
          className="p-4 border dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700/30 shadow-sm"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-grow min-w-0"> {/* Added min-w-0 for text truncation if needed */}
              <p className="font-semibold text-gray-800 dark:text-gray-100 truncate" title={member.name || `${member.firstName} ${member.lastName}`}>
                {member.name || `${member.firstName} ${member.lastName}` || 'Unnamed Member'}
              </p>
              {member.email && (
                <p className="text-sm text-gray-600 dark:text-gray-400 truncate" title={member.email}>
                  {member.email}
                </p>
              )}
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                Role: {member.role_in_tree || member.relationshipToUser || 'N/A'}
              </p>
              {member.dateOfBirth && (
